...
const palettePreviews: Record<
  PaletteId,
  { start: string; end: string; chip: string }
> = {
  mono: { start: '#ffffff', end: '#171717', chip: '#ffffff33' },
...
export const DashboardThemeSettingsPage = ({
  ...
  const handlePaletteChange = async (nextPaletteId: PaletteId) => {
    if (
      nextPaletteId === selectedPaletteId ||
      updatePalettePreference.isPending
    ) {
      return;
    }
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-x-4 bottom-4 flex gap-2">
                    <span
                      className="h-8 flex-1 rounded-md border border-white/30"
