...
export const useCreateCompany = (apiBaseUrl?: string) => {
  ...
  return useMutation({
    mutationFn: async (payload: CreateCompanyPayload) => repository.createCompany(payload),
    onSuccess: async () => {
      const session = await repository.getAuthSession();

      setSession(session);
      queryClient.setQueryData(authQueryKey, session);
      await queryClient.invalidateQueries({ queryKey: palettePreferenceQueryKey });
    },
  });
};
...
export const useUpdatePalettePreference = (apiBaseUrl?: string) => {
  ...
  return useMutation({
    mutationFn: (paletteId: PaletteId) => repository.updatePalettePreference(paletteId),
