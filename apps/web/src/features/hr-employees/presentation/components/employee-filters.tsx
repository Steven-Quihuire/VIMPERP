...
const statusOptions: { value: EmploymentStatus; label: string }[] = [
  ...
  { value: 'separated', label: 'Desvinculados' },
];
...
export const EmployeeFilters = ({
  ...
  const activeCount = value.status.size + (value.withEmailOnly ? 1 : 0);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <div className="max-h-72 overflow-y-auto px-4 py-3">
          ...
          <label className="flex cursor-pointer items-center gap-2 rounded-lg px-1 py-1.5 hover:bg-muted/60">
            <Checkbox
              checked={value.withEmailOnly}
              onCheckedChange={(next) =>
                onChange({ ...value, withEmailOnly: Boolean(next) })
              }
            />
            <span className="text-sm">Solo con correo</span>
          </label>
        </div>
      </PopoverContent>
    </Popover>
