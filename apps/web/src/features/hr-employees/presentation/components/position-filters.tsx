...
export type PositionFiltersValue = {
...
export const PositionFilters = ({
  ...
  value: PositionFiltersValue;
  onChange: (next: PositionFiltersValue) => void;
  onClear: () => void;
}) => {
  const [open, setOpen] = useState(false);
  const activeCount = value.active.size;
        ...
        <div className="px-4 py-3">
          ...
          <div className="space-y-1">
            {statusOptions.map((option) => {
              const checked = value.active.has(option.value);
              return (
                <label
                  key={option.value}
